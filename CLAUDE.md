...
Правила проверок после разбора критика и CDO 07.09: ПРОЗА ищется после снятия вёрстки
(переносы строк не рвут фразу), АТРИБУТЫ и адреса в сыром html; у пунктов с ожидаемым нулём
(имена, цены, тире) контроли печатаются при каждом запуске (подсадки краснеют, законный похожий
текст молчит); цены по образцу критика [prism96] (сум/сўм/so'm/UZS/$/USD с любой стороны числа,
множитель только между числом и валютой), тире как пунктуация `[—–―−]` (CDO [harvest74]); голое
слово easy именем не считается; `states.py` требует «18 из 18 измерилось» и 0 расхождений
(критик [fern70]); рядом с путём героя печатается общий вес страницы по всем ссылкам (CDO [lava57]).
Адрес телеграма живёт в разметке обеих `a.btn.tg`, не в скрипте (CDO [meteor33]); проверка CDO
`warroom/cdo/checks/kassa-tg-links.py` требует Playwright, здесь его нет.
